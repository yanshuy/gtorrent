...
-type message(GSZ) :: {packet, bitstring()} | {user, GSZ}.
...
-type connection(GTA) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(GTA))}.

-opaque next(GTB, GTC) :: {continue,
        GTB,
        gleam@option:option(gleam@erlang@process:selector(GTC))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(GTD, GTE) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(GTE)) -> {GTD,
            gleam@option:option(gleam@erlang@process:selector(GTE))}),
        fun((GTD, message(GTE), connection(GTE)) -> next(GTD, message(GTE))),
        gleam@option:option(fun((GTD) -> nil)),
        integer(),
        boolean(),
        boolean(),
...
-spec continue(GUE) -> next(GUE, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 151).
-spec with_selector(next(GUI, GUJ), gleam@erlang@process:selector(GUJ)) -> next(GUI, GUJ).
...
?DOC(false).
-spec convert_next(next(GUX, GUY)) -> glisten@internal@handler:next(GUX, GUY).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(GVD, GVE), fun((GVE) -> GVH)) -> next(GVD, GVH).
...
-spec map_user_selector(gleam@erlang@process:selector(message(GVK))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(GVK)).
...
-spec convert_loop(
    fun((GVP, message(GVQ), connection(GVQ)) -> next(GVP, message(GVQ)))
) -> fun((GVP, glisten@internal@handler:loop_message(GVQ), glisten@internal@handler:connection(GVQ)) -> glisten@internal@handler:next(GVP, glisten@internal@handler:loop_message(GVQ))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(GVV)) -> {GVX,
        gleam@option:option(gleam@erlang@process:selector(GVV))})
) -> fun((glisten@internal@handler:connection(GVV)) -> {GVX,
    gleam@option:option(gleam@erlang@process:selector(GVV))}).
...
-spec new(
    fun((connection(GWD)) -> {GWF,
        gleam@option:option(gleam@erlang@process:selector(GWD))}),
    fun((GWF, message(GWD), connection(GWD)) -> next(GWF, message(GWD)))
) -> builder(GWF, GWD).
new(On_init, Loop) ->
...
-spec with_close(builder(GWM, GWN), fun((GWM) -> nil)) -> builder(GWM, GWN).
...
-spec with_pool_size(builder(GWS, GWT), integer()) -> builder(GWS, GWT).
...
?DOC(false).
-spec with_http2(builder(GWY, GWZ)) -> builder(GWY, GWZ).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_ipv6(builder(GXK, GXL)) -> builder(GXK, GXL).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(GXQ, GXR), binary(), binary()) -> builder(GXQ, GXR).
...
-spec bind(builder(GXE, GXF), binary()) -> builder(GXE, GXF).
