...
-spec be_ok({ok, GZP} | {error, any()}) -> GZP.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, GZU}) -> GZU.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(GZX)) -> GZX.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
