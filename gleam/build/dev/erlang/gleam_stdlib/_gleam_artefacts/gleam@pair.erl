...
-spec map_first({CMO, CMP}, fun((CMO) -> CMQ)) -> {CMQ, CMP}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CMR, CMS}, fun((CMS) -> CMT)) -> {CMR, CMT}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
