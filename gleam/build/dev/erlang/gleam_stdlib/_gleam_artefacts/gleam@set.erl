...
-opaque set(CVU) :: {set, gleam@dict:dict(CVU, list(nil))}.
...
-spec insert(set(CWC), CWC) -> set(CWC).
insert(Set, Member) ->
...
-spec contains(set(CWF), CWF) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CWH), CWH) -> set(CWH).
delete(Set, Member) ->
...
-spec from_list(list(CWN)) -> set(CWN).
from_list(Members) ->
...
-spec fold(set(CWQ), CWS, fun((CWS, CWQ) -> CWS)) -> CWS.
...
-spec filter(set(CWT), fun((CWT) -> boolean())) -> set(CWT).
...
-spec map(set(CWW), fun((CWW) -> CWY)) -> set(CWY).
map(Set, Fun) ->
...
-spec drop(set(CXA), list(CXA)) -> set(CXA).
drop(Set, Disallowed) ->
...
-spec take(set(CXE), list(CXE)) -> set(CXE).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 290).
-spec order(set(CXM), set(CXM)) -> {set(CXM), set(CXM)}.
order(First, Second) ->
...
-spec union(set(CXI), set(CXI)) -> set(CXI).
union(First, Second) ->
...
-spec intersection(set(CXR), set(CXR)) -> set(CXR).
...
-spec difference(set(CXV), set(CXV)) -> set(CXV).
...
-spec is_subset(set(CXZ), set(CXZ)) -> boolean().
...
-spec is_disjoint(set(CYC), set(CYC)) -> boolean().
...
-spec symmetric_difference(set(CYF), set(CYF)) -> set(CYF).
...
-spec each(set(CYJ), fun((CYJ) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
