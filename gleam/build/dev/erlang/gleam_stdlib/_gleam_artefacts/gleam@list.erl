...
-spec repeat_loop(ALR, integer(), list(ALR)) -> list(ALR).
...
-spec split_loop(list(ALY), integer(), list(ALY)) -> {list(ALY), list(ALY)}.
...
-spec split(list(ALU), integer()) -> {list(ALU), list(ALU)}.
split(List, Index) ->
    split_loop(List, Index, []).

-file("src/gleam/list.gleam", 1471).
-spec split_while_loop(list(AMH), fun((AMH) -> boolean()), list(AMH)) -> {list(AMH),
    list(AMH)}.
...
-spec split_while(list(AMD), fun((AMD) -> boolean())) -> {list(AMD), list(AMD)}.
...
-spec key_find(list({AMM, AMN}), AMM) -> {ok, AMN} | {error, nil}.
...
-spec key_filter(list({AMR, AMS}), AMR) -> list(AMS).
...
-spec key_pop_loop(list({ANB, ANC}), ANB, list({ANB, ANC})) -> {ok,
        {ANC, list({ANB, ANC})}} |
    {error, nil}.
...
-spec key_set_loop(list({ANM, ANN}), ANM, ANN, list({ANM, ANN})) -> list({ANM,
    ANN}).
...
-spec key_set(list({ANI, ANJ}), ANI, ANJ) -> list({ANI, ANJ}).
...
-spec each(list(ANR), fun((ANR) -> any())) -> nil.
each(List, F) ->
    case List of
        [] ->
            nil;
...
-spec try_each(list(ANU), fun((ANU) -> {ok, any()} | {error, ANX})) -> {ok, nil} |
    {error, ANX}.
try_each(List, Fun) ->
    case List of
        [] ->
...
-spec partition_loop(list(BGS), fun((BGS) -> boolean()), list(BGS), list(BGS)) -> {list(BGS),
    list(BGS)}.
...
-spec partition(list(AOC), fun((AOC) -> boolean())) -> {list(AOC), list(AOC)}.
...
-spec permutation_zip(list(AOP), list(AOP), list(list(AOP))) -> list(list(AOP)).
...
-spec permutations(list(AOL)) -> list(list(AOL)).
permutations(List) ->
    case List of
        [] ->
            [[]];
...
-spec window_loop(list(list(APJ)), list(APJ), integer()) -> list(list(APJ)).
...
-spec window(list(APF), integer()) -> list(list(APF)).
window(List, N) ->
    case N =< 0 of
        true ->
            [];
...
-spec window_by_2(list(APP)) -> list({APP, APP}).
window_by_2(List) ->
...
-spec drop_while(list(APS), fun((APS) -> boolean())) -> list(APS).
...
-spec take_while_loop(list(APY), fun((APY) -> boolean()), list(APY)) -> list(APY).
...
-spec take_while(list(APV), fun((APV) -> boolean())) -> list(APV).
take_while(List, Predicate) ->
    take_while_loop(List, Predicate, []).

-file("src/gleam/list.gleam", 1863).
-spec chunk_loop(list(AQH), fun((AQH) -> AQJ), AQJ, list(AQH), list(list(AQH))) -> list(list(AQH)).
...
-spec chunk(list(AQC), fun((AQC) -> any())) -> list(list(AQC)).
chunk(List, F) ->
    case List of
        [] ->
            [];
...
-spec sized_chunk(list(AQP), integer()) -> list(list(AQP)).
...
-spec reduce(list(ARA), fun((ARA, ARA) -> ARA)) -> {ok, ARA} | {error, nil}.
reduce(List, Fun) ->
    case List of
        [] ->
            {error, nil};
...
-spec scan_loop(list(ARI), ARK, list(ARK), fun((ARK, ARI) -> ARK)) -> list(ARK).
...
-spec scan(list(ARE), ARG, fun((ARG, ARE) -> ARG)) -> list(ARG).
...
-spec last(list(ARN)) -> {ok, ARN} | {error, nil}.
last(List) ->
    case List of
        [] ->
            {error, nil};
...
-spec combinations(list(ARR), integer()) -> list(list(ARR)).
...
-spec combination_pairs_loop(list(ARY), list({ARY, ARY})) -> list({ARY, ARY}).
...
-spec combination_pairs(list(ARV)) -> list({ARV, ARV}).
combination_pairs(Items) ->
    combination_pairs_loop(Items, []).

-file("src/gleam/list.gleam", 2107).
-spec take_firsts(list(list(ASS)), list(ASS), list(list(ASS))) -> {list(ASS),
    list(list(ASS))}.
...
-spec transpose_loop(list(list(ASL)), list(list(ASL))) -> list(list(ASL)).
...
-spec transpose(list(list(ASG))) -> list(list(ASG)).
...
-spec interleave(list(list(ASC))) -> list(ASC).
interleave(List) ->
    _pipe = List,
...
-spec shuffle_pair_unwrap_loop(list({float(), ATE}), list(ATE)) -> list(ATE).
...
-spec do_shuffle_by_pair_indexes(list({float(), ATI})) -> list({float(), ATI}).
...
-spec shuffle(list(ATB)) -> list(ATB).
shuffle(List) ->
    _pipe = List,
...
-spec max_loop(list(ATS), fun((ATS, ATS) -> gleam@order:order()), ATS) -> ATS.
...
-spec max(list(ATL), fun((ATL, ATL) -> gleam@order:order())) -> {ok, ATL} |
    {error, nil}.
max(List, Compare) ->
    case List of
        [] ->
...
-spec build_reservoir_loop(
    list(AUH),
    integer(),
    gleam@dict:dict(integer(), AUH)
) -> {gleam@dict:dict(integer(), AUH), list(AUH)}.
...
-spec build_reservoir(list(AUC), integer()) -> {gleam@dict:dict(integer(), AUC),
    list(AUC)}.
...
-spec sample(list(ATT), integer()) -> list(ATT).
sample(List, N) ->
