...
-spec map({ok, CNF} | {error, CNG}, fun((CNF) -> CNJ)) -> {ok, CNJ} |
    {error, CNG}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CNM} | {error, CNN}, fun((CNN) -> CNQ)) -> {ok, CNM} |
    {error, CNQ}.
...
-spec flatten({ok, {ok, CNT} | {error, CNU}} | {error, CNU}) -> {ok, CNT} |
    {error, CNU}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, COB} | {error, COC}, fun((COB) -> {ok, COF} | {error, COC})) -> {ok,
        COF} |
    {error, COC}.
'try'(Result, Fun) ->
    case Result of
...
-spec unwrap({ok, COK} | {error, any()}, COK) -> COK.
...
-spec lazy_unwrap({ok, COO} | {error, any()}, fun(() -> COO)) -> COO.
...
-spec unwrap_error({ok, any()} | {error, COT}, COT) -> COT.
...
-spec 'or'({ok, COW} | {error, COX}, {ok, COW} | {error, COX}) -> {ok, COW} |
    {error, COX}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CPE} | {error, CPF}, fun(() -> {ok, CPE} | {error, CPF})) -> {ok,
        CPE} |
    {error, CPF}.
...
-spec all(list({ok, CPM} | {error, CPN})) -> {ok, list(CPM)} | {error, CPN}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 307).
-spec partition_loop(list({ok, CQB} | {error, CQC}), list(CQB), list(CQC)) -> {list(CQB),
    list(CQC)}.
...
-spec partition(list({ok, CPU} | {error, CPV})) -> {list(CPU), list(CPV)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CQK}, CQN) -> {ok, CQN} | {error, CQK}.
...
-spec replace_error({ok, CQQ} | {error, any()}, CQU) -> {ok, CQQ} | {error, CQU}.
...
-spec values(list({ok, CQX} | {error, any()})) -> list(CQX).
values(Results) ->
