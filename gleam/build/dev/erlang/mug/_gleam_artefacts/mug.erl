    ...
    case erlang:element(5, Options) of
        ipv4_only ->
            ...
            gleam@result:map_error(
                _pipe,
                fun(Field@0) -> {connect_failed_ipv4, Field@0} end
            );
        ...
        ipv6_only ->
            ...
            gleam@result:map_error(
                _pipe@1,
                fun(Field@0) -> {connect_failed_ipv6, Field@0} end
            );
...
-spec map_tcp_message(fun((tcp_message()) -> HCM)) -> fun((gleam@dynamic:dynamic_()) -> HCM).
...
-spec select_tcp_messages(
    gleam@erlang@process:selector(HCJ),
    fun((tcp_message()) -> HCJ)
) -> gleam@erlang@process:selector(HCJ).
